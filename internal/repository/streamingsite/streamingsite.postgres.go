...
func (r *postgresRepository) Create(ctx context.Context, s *domain.StreamingSite) (int64, error) {
	const q = `
		INSERT INTO streaming_sites (profile_id, name, url, language, position, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
		RETURNING id`

	var id int64
	err := r.db.QueryRowContext(ctx, q,
		s.ProfileID(), s.Name(), s.URL(), string(s.Language()), s.Position(), s.CreatedAt(), s.UpdatedAt(),
	).Scan(&id)
	if err != nil {
...
func (r *postgresRepository) GetAllByProfileID(ctx context.Context, profileID int64) ([]*domain.StreamingSite, error) {
	const q = `
		SELECT id, profile_id, name, url, language, position, created_at, updated_at
...
func (r *postgresRepository) Update(ctx context.Context, s *domain.StreamingSite) error {
	...
	res, err := r.db.ExecContext(ctx, q, s.Name(), s.URL(), string(s.Language()), s.Position(), time.Now().UTC(), s.ID())
	if err != nil {
...
func scanSite(row rowScanner) (*domain.StreamingSite, error) {
	...
	if err := row.Scan(&id, &profileID, &name, &url, &languageS, &position, &createdAt, &updatedAt); err != nil {
	...
	return domain.Reconstitute(id, profileID, name, url, domain.Language(languageS), position, createdAt, updatedAt), nil
}
